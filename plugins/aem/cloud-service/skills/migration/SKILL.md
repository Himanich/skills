...
2. **Pin behavior first (before-eligible patterns only).** If the active pattern is before-eligible in [references/characterization-tests.md](references/characterization-tests.md) (`guavaCache`, `scheduler`, `resourceChangeListener`, `eventListener`/`eventHandler`), write **one** characterization test for this finding and run it green **before** editing — this pins current behavior. Follow that file's **efficiency contract exactly**: one fill-in skeleton, existing test harness only, one attempt, **skip-fast** (no stable seam / no harness / not green first run → discard the test, record `pin-skip`). Never debug a test or add dependencies. All other patterns (including `replication`, `assetApi`, and every non-Java pattern) skip this step.
